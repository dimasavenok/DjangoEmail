...
class MailingHomeView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        data = False#cache.get(cache_key)
        if not data:
            total_mailings = Mailing.objects.filter(owner=user).count()
            active_mailings = Mailing.objects.filter(owner=user, status=Mailing.STATUS_RUNNING).count()
            unique_recipients = Recipient.objects.filter(owner=user).count()
            # статистика попыток:
            success_count = MailingAttempt.objects.filter(mailing__owner=user,
                                                          status=MailingAttempt.STATUS_SUCCESS).count()
            failed_count = MailingAttempt.objects.filter(mailing__owner=user,
                                                         status=MailingAttempt.STATUS_FAILED).count()
            data = {
            ...
            # cache.set(cache_key, data, 60)  # кеш 60 секунд
...
class SendMailingView(LoginRequiredMixin, View):
    def post(self, request, pk):
        ...
        # проверка прав
        if mailing.owner != request.user: #and not request.user.groups.filter(name='Managers').exists():
